...
- 네트워크 호출은 `requests`를 사용합니다. 코어 requirements.txt에는
  beautifulsoup4는 포함되어 있지만 requests는 빠져있어(services/
  metadata_factory.py가 코어 제공으로 "가정"만 할 뿐 실제로는 설치돼 있지
  않을 수 있음), 이 플러그인 폴더에 `requirements.txt`(requests)를 함께 두어
  코어의 자동 설치 로직(ensure_plugin_dependencies)이 plugins/metadata/
  jikji_sf/libs/ 에 격리 설치하도록 합니다.
"""
...
# 일부 오래된 사이트는 기본 User-Agent(python-requests/x.y)를 차단합니다.
REQUEST_HEADERS = {
    "User-Agent": (
...
class JikjiSFMetadataProvider(BaseMetadataProvider):
    ...
    def _download_listview_html(self):
        """requests로 목록 페이지를 내려받아 EUC-KR로 디코딩합니다."""
        resp = requests.get(LISTVIEW_URL, headers=REQUEST_HEADERS, timeout=15)
        resp.raise_for_status()

        raw = resp.content
        # 원본 페이지는 EUC-KR 인코딩입니다. euc-kr 디코딩이 실패하면 cp949로
        # 재시도합니다 (거의 상위호환이라 EUC-KR에 없는 확장 조합만 다를 수 있음).
